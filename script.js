document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function validateField(field) {
      ...
      if (field.minLength && field.minLength > 0 && value.length < field.minLength) {
        showFieldError(field, label + ' must be at least ' + field.minLength + ' characters.');
        return false;
      }

      if (field.maxLength && field.maxLength > 0 && value.length > field.maxLength) {
        showFieldError(field, label + ' must not exceed ' + field.maxLength + ' characters.');
        return false;
      }
  ...
  const animatableElements = document.querySelectorAll(
    '.fade-in, .animate-on-scroll, [data-animate], .section-animate, ' +
    '.hero-content, .feature-card, .service-card, .pricing-card, ' +
    '.testimonial-card, .team-card, .about-content, .cta-section'
  );
  ...
  if (animatableElements.length > 0) {
    ...
    function applyInitialStyles(el) {
      ...
      el.style.cssText += ';' + defaultStyles + ';transition-delay:' + delay + 'ms';
    }
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "Y.sistem משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '3fe1076c-940b-409e-95b7-175dc9097318',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '3fe1076c-940b-409e-95b7-175dc9097318';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '3fe1076c-940b-409e-95b7-175dc9097318';
